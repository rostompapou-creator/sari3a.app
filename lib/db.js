...
import { getInitialAdminConfig, isDemoDataEnabled, shouldForceAdminSync } from "./runtime.js"
...
async function ensureAppSettingsSchema() {
  const columns = await all("PRAGMA table_info(app_settings)")
  const hasShipmentStatuses = columns.some((column) => column.name === "shipment_statuses")

  if (!hasShipmentStatuses) {
    await run("ALTER TABLE app_settings ADD COLUMN shipment_statuses TEXT")
  }
}
...
async function ensureAppSettings() {
  ...
  await run(
    `INSERT INTO app_settings (
      id, brand_name, tagline, support_phone, support_email, hero_title, hero_description, shipment_statuses, primary_color, secondary_color, updated_at
    ) VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      "Sari3a Delivery",
...
async function ensureAdminAccount() {
  ...
  const forcedSync = shouldForceAdminSync()
  const currentAdmin = await get("SELECT id, email FROM users WHERE role = 'admin' AND email = ?", [config.email])
  const anyAdmin = await get("SELECT id, email FROM users WHERE role = 'admin' ORDER BY id ASC LIMIT 1")

  if (forcedSync) {
    const userWithConfiguredEmail = await get("SELECT id, role FROM users WHERE email = ?", [config.email])
    if (userWithConfiguredEmail && userWithConfiguredEmail.role !== "admin") {
      throw new Error(
        `Impossible de synchroniser l'admin: l'email ${config.email} appartient deja a un utilisateur ${userWithConfiguredEmail.role}.`
      )
    }
    ...
    if (currentAdmin) {
      await run(
        `UPDATE users
         SET full_name = ?, phone = ?, password_hash = ?, governorate = ?, address = ?, status = 'active',
             current_lat = ?, current_lng = ?, updated_at = ?
         WHERE id = ?`,
        [config.full_name, config.phone, hashPassword(config.password), config.governorate, config.address, 36.8065, 10.1815, isoDate(), currentAdmin.id]
      )
      return
    }
    ...
    if (anyAdmin) {
      await run(
        `UPDATE users
         SET full_name = ?, email = ?, phone = ?, password_hash = ?, governorate = ?, address = ?, status = 'active',
             current_lat = ?, current_lng = ?, updated_at = ?
         WHERE id = ?`,
        [config.full_name, config.email, config.phone, hashPassword(config.password), config.governorate, config.address, 36.8065, 10.1815, isoDate(), anyAdmin.id]
      )
      return
    }
  } else {
