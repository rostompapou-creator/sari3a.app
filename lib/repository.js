...
const progressStatusKeys = new Set(statusOrder)
...
function normalizeAdditionalShipmentStatuses(statuses) {
  ...
  return statuses
    .map((status) => {
      ...
      const key = slugifyStatusLabel(status?.key || label)
      if (!label || !key || progressStatusKeys.has(key) || seen.has(key)) return null
      seen.add(key)
      return { key, label, type: "manual" }
    })
    .filter(Boolean)
}

function getConfiguredShipmentStatuses(settings) {
  return [...STATUS_STEPS, ...normalizeAdditionalShipmentStatuses(settings?.shipment_statuses)]
}

function getShipmentStatusLabelMap(settings) {
  return Object.fromEntries(getConfiguredShipmentStatuses(settings).map((status) => [status.key, status.label]))
}

function ensureAllowedShipmentStatus(status, settings) {
  if (!status) return null
  const allowedStatuses = new Set(getConfiguredShipmentStatuses(settings).map((item) => item.key))
  if (!allowedStatuses.has(status)) throw new Error("Statut colis invalide")
  return status
}

function buildFinancialOverview(shipments) {
  const aggregate = (keyField, nameField, fallbackLabel) =>
    Object.values(
      shipments.reduce((accumulator, shipment) => {
        const key = shipment[keyField]
        if (!key) return accumulator

        if (!accumulator[key]) {
          accumulator[key] = {
            id: key,
            name: shipment[nameField] || fallbackLabel,
            shipments: 0,
            amount: 0
          }
        }

        accumulator[key].shipments += 1
        accumulator[key].amount += Number(shipment.delivery_fee || 0)
        return accumulator
      }, {})
    )
      .sort((left, right) => right.amount - left.amount || right.shipments - left.shipments || left.name.localeCompare(right.name))

  return {
    totalAmount: shipments.reduce((sum, shipment) => sum + Number(shipment.delivery_fee || 0), 0),
    byPartner: aggregate("partner_id", "partner_name", "Partenaire"),
    byDriver: aggregate("driver_id", "driver_name", "Livreur")
  }
}
...
export async function getPortalData(session) {
  ...
  return {
    ...
    financials: buildFinancialOverview(shipments),
    ...
    admins,
    settings,
    statuses: getConfiguredShipmentStatuses(settings),
    driverApplications,
    partnerApplications,
...
export async function getAppSettings() {
  await initializeDatabase()
  const settings = await get("SELECT * FROM app_settings WHERE id = 1")
  if (!settings) return null

  let shipmentStatuses = []
  try {
    shipmentStatuses = normalizeAdditionalShipmentStatuses(JSON.parse(settings.shipment_statuses || "[]"))
  } catch {
    shipmentStatuses = []
  }
...
async function getShipmentDetailsByTrackingNumber(trackingNumber) {
  return get(
    `SELECT
      s.*,
      p.full_name AS partner_name,
...
async function addEvent(shipmentId, status, description, actorRole, actorName, lat, lng) {
  const labels = getShipmentStatusLabelMap(await getAppSettings())
  await run(
    `INSERT INTO shipment_events (shipment_id, status, label, description, actor_role, actor_name, lat, lng, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [shipmentId, status, labels[status] ?? TRACKING_LABELS[status] ?? status, description, actorRole, actorName, lat ?? null, lng ?? null, nowIso()]
  )
}
...
export async function createShipment(session, payload) {
  ...
  const settings = await getAppSettings()
  const initialStatus = ensureAllowedShipmentStatus(payload.status, settings) ?? "pending"
  ...
  await addEvent(result.lastID, initialStatus, "Colis cree dans la plateforme Sari3a", session.role, session.user.full_name, pickupLat, pickupLng)
...
export async function updateShipment(session, shipmentId, payload) {
  ...
  const fields = {
    ...
    status:
      session.role === "admin" || session.role === "partner"
        ? ensureAllowedShipmentStatus(payload.status ?? shipment.status, settings) ?? shipment.status
        : shipment.status
  }
  ...
  await run(
    `UPDATE shipments
     SET title = ?, description = ?, recipient_name = ?, recipient_phone = ?, recipient_address = ?,
         ...
         driver_id = ?, pickup_lat = ?, pickup_lng = ?, delivery_lat = ?, delivery_lng = ?, notes = ?, status = ?, updated_at = ?
     WHERE id = ?`,
    [
      fields.title,
      fields.description,
  ...
  if (fields.status !== shipment.status) {
    const labels = getShipmentStatusLabelMap(settings)
    await addEvent(
      shipmentId,
      fields.status,
      `Statut mis a jour manuellement vers ${labels[fields.status] ?? fields.status}`,
      session.role,
...
export async function advanceShipmentStatus(session, shipmentId) {
  ...
  await addEvent(shipmentId, nextStatus, `Statut passe a ${labels[nextStatus] ?? TRACKING_LABELS[nextStatus] ?? nextStatus}`, session.role, session.user.full_name, currentLat, currentLng)
  return getShipmentById(shipmentId)
}

export async function getShipmentForDeliveryScan(trackingNumber) {
  await initializeDatabase()
  if (!trackingNumber) return null
  return getShipmentDetailsByTrackingNumber(trackingNumber)
}

export async function confirmShipmentDelivery(session, trackingNumber) {
  await initializeDatabase()
  if (!["driver", "admin", "partner"].includes(session.role)) {
    throw new Error("Confirmation de livraison non autorisee")
  }

  const shipment = await getShipmentDetailsByTrackingNumber(trackingNumber)
  assertCanAccessShipment(session, shipment)
  ...
  await run(
    `UPDATE shipments
     SET status = 'delivered', current_lat = ?, current_lng = ?, updated_at = ?, delivered_at = ?
     WHERE id = ?`,
    [currentLat, currentLng, nowIso(), nowIso(), shipment.id]
  )

  await addEvent(shipment.id, "delivered", "Livraison confirmee apres scan du QR code", session.role, session.user.full_name, currentLat, currentLng)
  return getShipmentDetailsByTrackingNumber(trackingNumber)
}
...
export async function updateAppSettings(session, payload) {
  ...
  const fields = {
    ...
    shipment_statuses: normalizeAdditionalShipmentStatuses(payload.shipment_statuses ?? current.shipment_statuses),
  ...
  await run(
    `UPDATE app_settings
     SET brand_name = ?, tagline = ?, support_phone = ?, support_email = ?, hero_title = ?, hero_description = ?, shipment_statuses = ?, primary_color = ?, secondary_color = ?, updated_at = ?
     WHERE id = 1`,
    [
      fields.brand_name,
      fields.tagline,
      ...
      JSON.stringify(fields.shipment_statuses),
